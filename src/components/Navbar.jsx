...
import { logo, menu, close, bg } from '../assets';
    ...
    <nav className={`${styles.paddingX} w-full flex items-center py-5 fixed top-0 z-20 bg-primary`} >
      <div className='w-full flex justify-between items-center max-w-7xl mx-auto' >
        ...
        <Link to='/' className='flex items-center gap-2' onClick={ ()=>{ setActive(""); window.scroll(0,0) } } >
          <img src={bg} alt="logo" className='w-10 h-10 object-contain rounded-full' />
          <p className='text-white text-[18px] font-bold cursor-pointer flex'>Brayan &nbsp; <span className='sm:block hidden'> | Gamez </span> </p>
        </Link>

        <ul className='list-none hidden sm:flex flex-row gap-10'>
          {navLinks.map( (link)=>{
            return( //Los maps siempre deben devolver un RETURN 
